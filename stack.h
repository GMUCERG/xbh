...
#define NTOHS(n) (unsigned int)((((unsigned int) (n)) << 8) | (((unsigned int) (n)) >> 8))
#define NTOHS32(x) ((x & 0xFF000000)>>24)+((x & 0x00FF0000)>>8)+((x & 0x0000FF00)<<8)+((x & 0x000000FF)<<24)

#define NTOHS16(x) (((x & 0xFF00)>>8)+((x & 0x00FF)<<8))
#define HTONS16(x) (((x & 0xFF00)>>8)+((x & 0x00FF)<<8))
...
#define ORIGINAL_ETH_BUFFER_SIZE MTU_SIZE
#define SMALL_ETH_BUFFER_SIZE  UDP_HDR_LEN+ETH_HDR_LEN+255+1+50
...
//#define MAX_WINDOWS_SIZE (MTU_SIZE-100)	//This is the original, correct definition

/**
* This one is new. It seems that somewhere between Ubuntu 10.04LTS, the latest kernel updates,
* VirtualBox and it's drivers that TCP/IP protocol stack decided to split Jens' data transmission
* request from Perl into TCP packets of size MAX_WINDOWS_SIZE/2. And this was something the XBH
* (coming from a UDP tradition - one packet, one instruction including all prarameters) really
* took badly. It was not amused.
* Quick fix: report the wrong MAX_WINDOWS_SIZE, so that TCP packets may again be 1100 bytes big.
**/
#define MAX_WINDOWS_SIZE (2*MTU_SIZE-100)	
