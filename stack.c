...
unsigned char xbh_eth_buffer[ORIGINAL_ETH_BUFFER_SIZE+1];
unsigned char xbd_eth_buffer[SMALL_ETH_BUFFER_SIZE+1];
unsigned char *eth_buffer=xbh_eth_buffer;
unsigned int eth_buffer_size=ORIGINAL_ETH_BUFFER_SIZE;
	...
	if(eth.data_present)
	{
		...
		if ( (ReadRTL(RTL_ISR) & (1<<PRX)) != 0)
		{
			...
			{	
				...
				packet_length = ETH_PACKET_RECEIVE(eth_buffer_size,eth_buffer);
				if(packet_length > 0)
				{
				        //usart_write("Received packet of size %d (%d at %d)\n",packet_length, eth_buffer_size, eth_buffer);
...
void new_eth_header (unsigned char *buffer,unsigned long dest_ip)
{
	...
	if (b != MAX_ARP_ENTRY) //Eintrag gefunden wenn ungleich
	{
		for(unsigned char a = 0; a < 6; a++)
		{
			...
			ethernet->EnetPacketSrc[a] = eeprom_mac[a];
		}
		return;
	}
...
char arp_request (unsigned long dest_ip)
{
    ...
    {
        arp->ARP_SHAddr[count] = eeprom_mac[count];
                            ...
                            unsigned long dest_ip)
{
    ...
    udp->udp_Chksum = htons(result16);
		...
		//XBD_debugOutBuffer("Vor ETH_PACKET_SEND", eth_buffer,data_length);
...
void create_new_tcp_packet(unsigned int data_length,unsigned char index)
{
    ...
    //TCP Spec: Acknum shall only be set to nonzero value if ACK flag ist set!
